...
static const CGFloat kDefaultHeight = 120.0;
...
-(void)setUpViews
{
    ...
    setCornerForView(self.tableView, [UIColor clearColor], 0.0f, 5.0f);
...
-(void)performSetup
{
    ...
    {
        self.font = [UIFont fontWithName:@"Roboto-Regular" size:18];
        self.textColor = [UIColor colorWithRed:17/255.0f green:106/255.0f blue:152/255.0f alpha:1.0f];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
  ...
  //  CGFloat ht = [value getEstimatedHeightWithFont:self.font withWidth:viewFrame.size.width-60 withExtraHeight:2];
    
    NSDictionary *attributes = @{NSFontAttributeName: self.font};
    CGRect cellRect = [value boundingRectWithSize:CGSizeMake(self.tableView.frame.size.width-40.0,0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil];
    if (cellRect.size.height > height)
    {
        height = cellRect.size.height + 10.0;
    }
    return height;
