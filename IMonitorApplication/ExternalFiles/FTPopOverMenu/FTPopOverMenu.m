...
#define FTDefaultMenuFont                   [UIFont fontWithName:@"Roboto-Regular" size:15]
#define FTDefaultMenuWidth                  [[UIScreen mainScreen] bounds].size.width-100
#define FTDefaultMenuIconSize               24.f
#define FTDefaultMenuRowHeight              45.f
...
@property (nonatomic, strong) UILabel *menuImageNameLabel;
...
-(UILabel *)menuImageNameLabel
{
    if (!_menuImageNameLabel) {
        _menuImageNameLabel = [[UILabel alloc]initWithFrame:CGRectZero];
        _menuImageNameLabel.backgroundColor = [UIColor clearColor];
        [_menuImageNameLabel setFont:[UIFont fontWithName:@"imonitor" size:20]];
        self.menuImageNameLabel.textColor = [UIColor colorWithRed:(30.0/255.0) green:(161.0/255.0) blue:(222.0/255.0) alpha:1.0];

    }
    return _menuImageNameLabel;
}
...
-(void)setupWithMenuName:(NSString *)menuName menuImage:(id )menuImage
{
    ...
    if (!menuImage) {
        ...
        self.menuImageNameLabel.frame = iconImageRect;
        self.menuImageNameLabel.text = menuImage;
        self.menuImageNameLabel.textAlignment = configuration.textAlignment;
...
//                        }];
        ...
        [self.contentView addSubview:self.menuImageNameLabel];
    }
...
-(UITableView *)menuTableView
{
    if (!_menuTableView) {
        ...
        //Added to add border on table view
        _menuTableView.layer.borderWidth = 1.0;
        _menuTableView.layer.borderColor = [UIColor colorWithRed:(30.0/255.0) green:(161.0/255.0) blue:(222.0/255.0) alpha:1.0].CGColor;
...
-(void)showWithFrame:(CGRect )frame
    ...
    shouldAutoScroll:(BOOL)shouldAutoScroll
         headerTitle: (NSString *)headerTitleString
           ...
           doneBlock:(FTPopOverMenuDoneBlock)doneBlock
{
    ...
    _strHeaderTitle = headerTitleString;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == _menuStringArray.count-1) {
        menuCell.separatorInset = UIEdgeInsetsMake(0, self.bounds.size.width+25, 0, 0);
    }else{
        menuCell.separatorInset = UIEdgeInsetsMake(0, [FTPopOverMenuConfiguration defaultConfiguration].menuTextMargin+25, 0, [FTPopOverMenuConfiguration defaultConfiguration].menuTextMargin);
    }
    return menuCell;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UIView *headerViewTitle = [[UIView alloc]initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, self.frame.size.height)];
    headerViewTitle.backgroundColor = [UIColor whiteColor];
    UILabel *labelTitle = [[UILabel alloc]initWithFrame:CGRectMake(15.0, 0.0, self.frame.size.width-15, 40)];
    labelTitle.text = self.strHeaderTitle;
    if ([self.strHeaderTitle isEqualToString:@"CHANGE VIEW"]) {
        labelTitle.textColor = [UIColor darkTextColor];
        labelTitle.font = [UIFont fontWithName:@"Roboto-Regular" size:16];
    }
    else {
        labelTitle.textColor = [UIColor colorWithRed:(30.0/255.0) green:(161.0/255.0) blue:(222.0/255.0) alpha:1.0];
        labelTitle.font = [UIFont fontWithName:@"Roboto-Regular" size:16];
    }
    [headerViewTitle addSubview:labelTitle];
    
    UILabel *separatorLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 40.0, self.frame.size.width, 1)];
    separatorLabel.backgroundColor = [UIColor lightGrayColor];
    [headerViewTitle addSubview:separatorLabel];
...
@property (strong, nonatomic) NSString *strHeaderTitle;
...
{
    [[self sharedInstance] showForSender:sender senderFrame:CGRectNull withMenu:menuArray imageNameArray:nil headerTitle:headerTitle doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showForSender:(UIView *)sender
         withMenuArray:(NSArray<NSString*> *)menuArray
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:sender senderFrame:CGRectNull withMenu:menuArray imageNameArray:imageArray headerTitle:headerTitle doneBlock:doneBlock dismissBlock:dismissBlock];
...
+ (void) showFromEvent:(UIEvent *)event
         withMenuArray:(NSArray<NSString*> *)menuArray
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:[event.allTouches.anyObject view] senderFrame:CGRectNull withMenu:menuArray imageNameArray:nil headerTitle:headerTitle doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showFromSenderFrame:(CGRect )senderFrame
               withMenuArray:(NSArray<NSString*> *)menuArray
                ...
                dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:nil senderFrame:senderFrame withMenu:menuArray imageNameArray:nil headerTitle:headerTitle doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showFromSenderFrame:(CGRect )senderFrame
               withMenuArray:(NSArray<NSString*> *)menuArray
                ...
                dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:nil senderFrame:senderFrame withMenu:menuArray imageNameArray:imageArray headerTitle:headerTitle doneBlock:doneBlock dismissBlock:dismissBlock];
}
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.strHeaderTitle = headerTitle;
        [self adjustPopOverMenu];
    });
}
...
-(void)adjustPopOverMenu
{
    ...
    CGFloat menuHeight = [FTPopOverMenuConfiguration defaultConfiguration].menuRowHeight * self.menuArray.count + self.menuArrowHeight + 45;
...
-(void)prepareToShowWithMenuRect:(CGRect)menuRect menuArrowPoint:(CGPoint)menuArrowPoint shouldAutoScroll:(BOOL)shouldAutoScroll arrowDirection:(FTPopOverMenuArrowDirection)arrowDirection
{
    ...
    [_popMenuView showWithFrame:menuRect anglePoint:menuArrowPoint withNameArray:self.menuArray imageNameArray:self.menuImageArray shouldAutoScroll:shouldAutoScroll headerTitle:self.strHeaderTitle arrowDirection:arrowDirection doneBlock:^(NSInteger selectedIndex) {
