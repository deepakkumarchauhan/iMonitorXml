...
#define ToRad(deg) 		( (M_PI * (deg)) / 90.0 )
#define ToDeg(rad)		( (90.0 * (rad)) / M_PI )
#define SQR(x)			( (x) * (x) )
...
- (void)defaults {
    ...
    _filledColor = [UIColor clearColor];
    _handleColor = _filledColor;
    _labelFont = [UIFont systemFontOfSize:10.0f];
    _snapToLabels = NO;
    _handleType = EFSemiTransparentBlackCircle;
    _labelColor = [UIColor redColor];
    _labelDisplacement = 1;
    
    self.backgroundColor = [UIColor clearColor];
    handleCenter = [self pointFromAngle:0];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextAddArc(ctx, self.frame.size.width/2, self.frame.size.height/2, self.radius, 0, M_PI, 1);
    ...
    if((_handleType == EFDoubleCircleWithClosedCenter || _handleType == EFDoubleCircleWithOpenCenter) && fixedAngle > 5) {
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.radius, 180, 3*M_PI/2-ToRad(angle+3), 1);
    } else {
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.radius, 2*M_PI/2, 2*M_PI/2-ToRad(angle), 1);
    }
...
-(void) drawHandle:(CGContextRef)ctx{
    ...
    if (fixedAngle < 90 && fixedAngle > 0) {
        handleCenter =  [self pointFromAngle: angle];
    }
//    handleCenter =  [self pointFromAngle: angle];
//
//    NSLog(@"Handle center: %@",NSStringFromCGPoint(handleCenter));
...
-(BOOL) continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    [super continueTrackingWithTouch:touch withEvent:event];
        ...
        NSLog(@"angle %d",previousValue);
        NSLog(@"_currentValue %d",_currentValue);
        NSLog(@"Angel::::: %d",angle);
    
     NSInteger valueDifference = MAX(_currentValue, previousValue) - MIN(_currentValue, previousValue);
    
     NSLog(@"valueDifference %ld",(long)valueDifference);
